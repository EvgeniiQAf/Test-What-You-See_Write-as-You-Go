...
export function priorityToCustomPriority(priority) {
  if (priority === "High") {
    return 3;
  }
...
export function buildPreconditionsHtml(preconditionsEn) {
...
export function normalizeExpectedLine(line) {
  return String(line || "")
    .replace(/^\s*\d+(?:\.\d+)?[\)\.]?\s*/u, "")
    .trim();
}

export function formatExpectedLines(lines, stepIndex) {
...
export function buildExpectedHtml(expectedEn, stepIndex) {
...
export function splitExpectedText(value) {
...
export async function compressScreenshotDataUrl(dataUrl, { maxWidth = 960, maxHeight = 960, quality = 0.35 } = {}) {
  if (!dataUrl) {
    return null;
  }
...
export async function normalizeScreenshotList(items, limit = 1) {
...
export function normalizeSelectionItem(item) {
  return {
...
export function normalizeSelectionList(items) {
