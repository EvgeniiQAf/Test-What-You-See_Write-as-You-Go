...
import {
  ...
  generateClarificationReply,
  generateTestCasesFromElement,
...
export const generateTestCases = async (
  ...
  console.log("[VALIDATION OK] Calling OpenAI...");
  const startTime = Date.now();
  
  try {
    if (shouldAskForClarification(validationResult.data)) {
      const reply = await generateClarificationReply(validationResult.data);
    ...
    const result = await generateTestCasesFromElement(validationResult.data);
    const elapsed = Date.now() - startTime;
...
export const createTestmoCase = async (
  req: Request,
...
export const chatWithAssistant = async (
  ...
  try {
    ...
    if (!reply) {
      res.status(502).json({
        error: "Assistant returned empty response",
      });
      return;
    }
