...
3. TEST STEPS (КРОКИ) - IMPERATIVE ACTIVE VOICE:
   ...
   - RECORDED USER ACTIONS: If a list of chronological "Recorded user actions" is provided below, analyze them critically to compile the test steps. Filter out accidental clicks (e.g. clicks on empty static background wrapper tags, repeated duplicate clicks, or unintended page scrolls). Consolidate minor sequential interactions (e.g. typing values into multiple login input fields) into single logical steps. Focus on generating a clean, coherent test scenario that matches the core user flow and testing intent!
